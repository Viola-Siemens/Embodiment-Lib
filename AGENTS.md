...
│   │   │   ├── command/                # WP-8：InspectData、InspectReportBuilder（纯逻辑）、CommandPermissions、EmbodimentCommandTree、InspectCommand
...
│       ├── api/ config/ attach/ runtime/ memory/ command/ tool/     # 与主源集同构
...
| 调试命令（`command/`） | WP-8：`/embodimentlib inspect [<entity>]` + 别名 `/emb inspect`（两棵树由同一批节点构造器注册，故不可能漂移）。分层同「纯逻辑 + 薄适配器」：`InspectData`（**纯逻辑**报告数据；`ProfileView` 只有名字/协议/模型名——**结构上没有 api_key/base_url 字段**）、`InspectReportBuilder`（**纯逻辑**：报告全部文字 + 200 字符截断）、`CommandPermissions`（权限判定，`PermissionSet` 是函数式接口故可单测）、`EmbodimentCommandTree`（对命令源类型**泛型化**，单测可用哑 source 让真实 Brigadier 解析别名）、`InspectCommand`（读附着/注册表/配置/会话 → 组装数据 → `sendSuccess(..., false)` 只回执行者）。权限 = 操作员二级（`Permissions.COMMANDS_GAMEMASTER`，PLAN §8 决策 21）；`<entity>` 由 `EntityArgument` 承担名字/UUID/`@e` 三种写法，未给参数时回退「执行者自身 → 附近 10 格内最近已附着实体」并在报告首行注明来源 |
...
3. **当前进度**：WP-0 ✅、WP-1 ✅（配置系统：`AgentHostSide` + ModConfigSpec 双端 TOML + Profile 路由——routing 为 List、profiles 为 JSON 列表，见 §6 配置行）、WP-2 ✅（实体附着 + 双端注册表：`attach/` 包提供 `agent_type`/`session_id` 附着、`AgentRegistry` 双端单例、`RegistryEntry`、`AgentLifecycle` 生命周期钩子）、WP-3 ✅（代理运行时：`runtime/` 包提供 `EmbodiedAgent`、`AgentLoop` 双环、`ThreadBridge`/`ToolBridge` 游戏线程桥接、`ExecutionGuard` 串行化、`ModelFactory` 模型工厂）、WP-4 ✅（会话与记忆持久化：`memory/` 包提供 `SessionPaths`（纯逻辑布局与 id 校验）、`SessionSink`/`SessionData`（写穿的工作记忆与待办）、`SessionStore`（根解析 + Gson 原子写 + 缓存 + AgentScope 状态存储/历史查询）、`SessionLifecycle`（实体消失 → 写盘并摘缓存；存档/停服 → flushAll）；会话历史改由 `JsonFileAgentStateStore` 落在会话目录内）、WP-5 ✅（工具契约与基础设施：`tool/` 包提供 `EmbodiedToolBase`、`ToolContext`+`ToolContextScope`、`ToolResults`、`Griefing`、`BuiltinToolkit`、`ToolPermissionChecker`）、WP-6 ✅（P0 内置工具 12 个：`tool/perceive|loco|action|meta` 四个子包，每个工具 = `*Logic` 纯逻辑类 + `*Tool` 薄适配器；`api/event/AgentSayEvent`）、WP-7 ✅（P1 内置工具 11 个：新增 `tool/container` 子包；基础设施下沉 `tool/Slots`/`Containers`/`Blocks`/`BlockAccess`/`BlockCoordinates`、`perceive/ResourceId`；`BuiltinToolkit.create()` 已注册**全部 23 个**工具；唯一持久状态 `FollowService`/`FollowGoal` 由主类 `EmbodimentLib` 接线）、WP-8 ✅（调试命令：`command/` 包提供 `InspectData`/`InspectReportBuilder`（**纯逻辑**，报告逐行可断言）、`CommandPermissions`（操作员二级）、`EmbodimentCommandTree`（对命令源类型泛型化 → 别名可单测）、`InspectCommand`（五个来源 → 报告）；`/embodimentlib inspect [<entity>]` + `/emb inspect` 已接线到 `RegisterCommandsEvent`；全量 `test` **381 例 0 失败**）；下一步 **WP-9**（演示实体 `demo_agent` + 全部未闭合验收项的端到端验证），最后 **WP-10**（Addon 扩展面与分发）。
   ...
   ⚠️ **WP-8 遗留一项待真实环境复核**：命令树与权限判定的单测已覆盖「谓词不满足 → 解析期不可达」与等级语义（恰好二级放行 / 更高档放行 / 更低档拒绝），但「真实玩家的 op 等级映射为 `LevelBasedPermissionSet` 对应档位」需 WP-9 用非 op 玩家在 `runServer` 中冒烟一次；`talk` 子命令属 WP-9，届时在同一批节点构造器上追加。
   ⚠️ **源码缺口**：`net/neoforged/fml` 与 `com/mojang/brigadier` 的源码都不在 `../Sources-26.1.2/`（该目录只有 `net/minecraft`、`net/neoforged/neoforge`、`io/agentscope`），故 `FMLPaths.CONFIGDIR.get()`（WP-4 客户端会话根）、`ModConfig`/`ModConfigEvent`（WP-1）以及 Brigadier 的树/解析语义只能靠**编译 + 行为测试**核对；如需逐行核对请补充这两个模块的源码。权限相关的新 API（`PermissionSet`/`LevelBasedPermissionSet`）在 `net/minecraft` 下，已逐签名核对。
